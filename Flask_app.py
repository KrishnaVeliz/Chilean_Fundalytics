...
#comparar rentabilidades nueva pestaña 
@app.route('/comparar-rentabilidades', methods=['GET', 'POST'])
def comparar_rentabilidades():
    df = pd.read_csv('uploads/rentabilidades_acumuladas.csv', sep=";", index_col=None)
    df['fecha'] = pd.to_datetime(df['fecha'], format='%Y-%m-%d')
    fondos = df['Run Fondo'].unique()

    if request.method == 'POST':
        selected_fondos = request.form.getlist('fondos')
        fecha_datepicker = request.form.get('datepicker')

        if fecha_datepicker is None:
            fecha_datepicker = df['fecha'].max()
        else:
            fecha_datepicker = datetime.strptime(fecha_datepicker, '%m/%d/%Y')

        df_selected = df[df['Run Fondo'].isin(selected_fondos) & (df['fecha'] == fecha_datepicker)]
        plot = create_plot(df_selected)

        return render_template('comparar_rentabilidades.html', fondos=fondos, plot=plot)
...
#pestaña nueva
def create_plot(df):
    fig = px.line(df, x='fecha', y='Rentabilidad', color='Run Fondo', title='Comparación de Rentabilidades')
